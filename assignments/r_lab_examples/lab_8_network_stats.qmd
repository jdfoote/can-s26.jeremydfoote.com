...
knitr::opts_chunk$set(echo = F, warn=F, message=F)
...
bkfrab <- bkfrab |> as_tbl_graph()

bkfrab |>
  mutate(bw_centrality = centrality_betweenness()) |>
  as_tibble() |>
  ggplot() +
  geom_histogram(aes(x=bw_centrality)) +
  theme_minimal()
...
G |>
  ...
  as_tibble() |>
  ggplot() +
  geom_boxplot(aes(x=university, fill=university, y=betweenness)) +
  theme_minimal()

```


5. Come up with something else that you want to visualize, either as a histogram or as boxplots and create a visualization below.
